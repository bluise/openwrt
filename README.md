...
- **只有 `/`、`/rom`、`/overlay` 这三个挂载点算"运行系统在用"**：被它们占着的盘会被拒绝
  （最典型的是"squashfs 版 U 盘把内置盘的分区拿去当可写层"）。其它挂载点（含 `/mnt/sdaX`
  这种数据分区、甚至 `/boot`）都算可回收：安装器会把它们**列为候选并在写盘前自动卸载**，
  卸不掉才拒绝
- 真被拒绝时它会打印**哪个分区挂在哪**并给出三条办法：换 ext4 版 U 盘启动（可写层在 U 盘上）／
  清掉那块盘的分区表再重启／`openwrt-install --force --disk /dev/sdX`（会破坏运行中的系统，
  自动筛选永远保守，所以要显式指定盘）；启动盘永远拒绝

...
### 没有网络也能装：离线克隆安装（推荐给瘦客户机 / 无网环境）

写进 U 盘的是**系统本身**，U 盘里并没有 `.img.gz` 文件（ESP 只有 16 MiB 装不下，
rootfs 是只读 squashfs 也放不下）。所以安装器原来的逻辑是"找不到镜像文件就联网下载" ——
**没网就没法装**。

现在多了一条离线路径：**把当前运行的这块 U 盘整盘克隆到目标盘**。

- 菜单里选 `9`（克隆安装），或者找不到镜像时它**第一个**就会问你要不要这么做
- 命令行：`openwrt-install --clone`（可配 `--no-menu`，适合脚本）
- 完全不需要镜像文件、不需要网络
- 只复制到"最后一个分区的末尾 + 备份 GPT"，不会傻拷整块 U 盘
- 目标盘只需比这个长度大即可（本固件约 2.1 GB）
- **装完必须拔掉 U 盘再启动**：克隆盘的分区 UUID 和源盘相同，两块盘同时插着
  时引导可能挑到 U 盘那份（完成提示里会再强调一次）

> 这就是 iStoreOS 的做法：它的 `quickstart` → `Install X86` 直接把自己运行的那块盘
> `dd` 到内置盘（见其源码 `istoreos/quickstart` 的 `backend/cmd/backend/prompt.go`），
> 所以它也不需要联网。我们做成了等价的一步。
